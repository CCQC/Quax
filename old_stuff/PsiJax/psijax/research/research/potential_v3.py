...
print("HERE")
print(indices)
# Mixed shell-nuclear derivatives indices
# We want to know if a combination of shell and nuclear derivative mathces the overal target derivative.
# are just every combination of indices in shell derivatives block and nuclear derivatives block
#for i in range(len(desired_atom_indices)):
#  desired_atom_idx = desired_atom_indices[i]
#  for j in range(2): # shell atom loop
#      atom_idx = shell_atom_index_list[j]
#  for k in range(natom)): # nuclear deriv loop
#      nuc_idx = k 
      #if ((desired_atom_idx == atom_idx) and (atom_idx == nuc_idx)):
      #  indices.append(
