...
def build_overlap(geom, centers1, centers2, basis_data, primitive_locations):
    ...
    if s_orb: 
        ss_primitives = jax.vmap(ssmap, (0,0,0))(centers_bra[ssmask], centers_ket[ssmask], basis_data[ssmask])
        all_primitives = np.concatenate((all_primitives, ss_primitives))

    def psmap(centers_bra, centers_ket, basis_data):
        Ax, Ay, Az = centers_bra
        Cx, Cy, Cz = centers_ket
        alpha_bra, alpha_ket, c1, c2, bra_am, ket_am = basis_data
        args = (Ax, Ay, Az, Cx, Cy, Cz, alpha_bra, alpha_ket, c1, c2)
    ...
    if p_orb:
        psvmap = jax.vmap(psmap, (0,0,0))
        ps_primitives = psvmap(centers_bra[psmask], centers_ket[psmask], basis_data[psmask])
        sp_primitives = psvmap(centers_bra[spmask], centers_ket[spmask], basis_data[spmask])
        #ps_primitives = jax.vmap(psmap, (0,0,0))(centers_bra[psmask], centers_ket[psmask], basis_data[psmask])
    ...
    if p_orb:
        ppvmap = jax.vmap(ppmap, (0,0,0))
        pp_primitives = ppvmap(centers_bra[ppmask], centers_ket[ppmask], basis_data[ppmask])
