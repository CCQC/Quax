...
def overlap_dd(Ax, Ay, Az, Cx, Cy, Cz, alpha_bra, alpha_ket, c1, c2):
    ...
    first_term = np.vstack((dx_1, dy_1[-2:], dz_1[-1])) # this is always going to be analogous
    lower = overlap_ds(Ax, Ay, Az, Cx, Cy, Cz, alpha_bra, alpha_ket, c1, c2) # just have to figure out what this is, can do it empirically by inspecting what it is in the original formalism
# NOTE for higher AM, 'lower' would sometimes need to be multiplied by 2, 3 etc in the vstack below, i think!
    second_term = np.vstack((lower, np.zeros(6), np.zeros(6), lower, np.zeros(6), lower)) 
    return oot_alpha_ket * (first_term + second_term)
...
#
#    second_term = np.vstack((dx_2, dy_2[-2:], dz_2[-1]))
#    print(second_term)
...
#print(overlap_dp(*args))
#print(overlap_dd(*args))
