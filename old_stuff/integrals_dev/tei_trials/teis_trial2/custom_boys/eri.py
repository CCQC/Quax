...
def eri_ssss_0(A, B, C, D, aa, bb, cc, dd, coeff):
    ...
    return ssss, zeta, eta, P, Q, boys_arg

def eri_ssss(A, B, C, D, aa, bb, cc, dd, coeff):
    """ 
    Computes a [ss|ss] integral. Should not be used except to  
    """
    tmp_ssss, zeta, eta, P, Q, boys_arg = eri_ssss_0(A, B, C, D, aa, bb, cc, dd, coeff)
    #TODO HERE VARY THE BOYS FUNCTION
    #ssss = tmp_ssss * boys0(boys_arg)
    #ssss = tmp_ssss * boys(boys_arg)
    ssss = tmp_ssss * control(boys_arg)
    return ssss
...
#print('old (pp|pp)', len(str(jaxpr).splitlines()))

#print(eri_pppp(*args))

jaxpr = jax.make_jaxpr(jax.jacfwd(jax.jacfwd(eri_pppp)))(*args)
print('old (pp|pp)', len(str(jaxpr).splitlines()))
