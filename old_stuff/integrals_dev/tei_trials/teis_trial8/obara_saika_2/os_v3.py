...
def vrr(xyza,lmna,alphaa, xyzb,alphab, xyzc,lmnc,alphac, xyzd,alphad,M):
    ...
    with loops.Scope() as S:
        #S.vrr_terms = np.zeros((la+1,ma+1,na+1,lc+1,mc+1,nc+1,mtot+1)) 
...
xyza = np.tile(xyza,(1000,1))
xyzb = np.tile(xyzb,(1000,1))
xyzc = np.tile(xyzc,(1000,1))
xyzd = np.tile(xyzd,(1000,1))
print(xyza.shape)
lmna = np.tile(np.array([1,2,1]),(1000,1))
lmnb = np.tile(np.array([0,0,0]),(1000,1))
lmnc = np.tile(np.array([1,0,2]),(1000,1))
lmnd = np.tile(np.array([0,0,0]),(1000,1))
print(lmnd.shape)

alphaa = np.repeat(0.5,1000)
alphab = np.repeat(0.4,1000)
alphac = np.repeat(0.3,1000)
alphad = np.repeat(0.2,1000)
M = np.repeat(0, 1000)
