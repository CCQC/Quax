...
def vrr(superarg):
    ...
    with loops.Scope() as S:
        ...
        S.vrr_terms = np.zeros((4,4,4,4,4,4,mtot+1))
    ...
    #print(S.vrr_terms[:S.i,:S.j,:S.k,0,0,0,0])
        ...
        # NOTE the angular momentum in x of [xs|ys] is determined entirely by the angular momentum in y, 
        # so you could just deal with a single 3d array while building all [xs|ys]

        # Strategy: first do x component angular momentum transfer from center A to center C, then do y component, then z component
        # Loop over cx promotion [ (3,0,0) s | (0,0,0) s] --> [ (2,0,0) s | (1,0,0) s] , [ (1,0,0) s | (2,0,0) s], [ (0,0,0) s | (3,0,0) s] 
        S.q = 0
        for _ in S.while_range(lambda: S.q < Lc):
            tmp = deltax * S.vrr_terms[S.i - S.q,S.j,S.k, S.q, 0, 0] - zeta / eta * S.vrr_terms[S.i - S.q + 1, S.j,S.k,S.q,0,0]
            tmp += (S.i - S.q) / (2 * eta) * S.vrr_terms[S.i - S.q - 1, S.j,S.k,S.q,0,0] + (S.q / (2 * eta)) * S.vrr_terms[S.i - S.q,S.j,S.k, S.q - 1, 0, 0]  
        ...
        for _ in S.while_range(lambda: S.r < Mc):
            ...
            for _ in S.while_range(lambda: S.q < Lc + 1): # i think this +1 is needed, per 2 lines down. plus, its analogous to VRR
                # hmmm not sure about this mixing of increments Si Sq and Sj Sr at the same time... is that right?
                                                                                                  # this term should not add 1 to x component right?
                tmp = deltay * S.vrr_terms[S.i - S.q, S.j - S.r, S.k, S.q, S.r, 0] - zeta / eta * S.vrr_terms[S.i - S.q, S.j - S.r + 1, S.k,S.q, S.r,0]
                # isnt this last term wrong? The prefix is only 0 when S.r is 0, but S.q - 1 can go out of range.. unless you add one to it?
                                                 # here also, should not reduce 1 in x component                         # only reduce in y component here as well 
                tmp += (S.j - S.r) / (2 * eta) * S.vrr_terms[S.i - S.q, S.j - S.r - 1,S.k,S.q,S.r,0] + (S.r / (2 * eta)) * S.vrr_terms[S.i - S.q, S.j - S.r, S.k, S.q, S.r - 1, 0]
