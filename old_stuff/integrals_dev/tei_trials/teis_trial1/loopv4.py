...
def compute(geom, coeffs, exps, atoms, am, indices):
    with loops.Scope() as s:
        ...
        def compute_primitive(A, B, C, D, aa, bb, cc, dd, coeff, am):
        ...
        vectorized_primitive = jax.vmap(compute_primitive, (None,None,None,None,0,0,0,0,0,None))
        ...
        def contraction(A, B, C, D, aa, bb, cc, dd, coeff, am):
            ...
            contraction = np.sum(primitives, axis=0).reshape(-1)
            contraction = np.pad(contraction, (0,81 - contraction.shape[0]))
            return contraction
