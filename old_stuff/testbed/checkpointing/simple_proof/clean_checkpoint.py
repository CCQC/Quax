...
class MyCheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        idx = detached_inputs[1] # index argument, controls which hessian row to compute
        # Pass only arguments of function, not index argument
        with torch.enable_grad():
            output = ctx.run_function(detached_inputs[0])

        # Now backward once for gradient, backward again for row of hessian
        output.backward(create_graph=True)
        gradient = detached_inputs[0].grad.clone().flatten()
        print("hack gradient", gradient)
