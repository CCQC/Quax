...
class RHF(nn.Module):
    ...
    def hf_iter(self, H, A, G, Enuc):
        ...
        C = torch.matmul(A, C2)
        Cocc = C[:, :self.ndocc]
        self.D = torch.einsum('pi,qi->pq', Cocc, Cocc)
    ...
    def forward(self, geom, row_idx):
        ...
        #D = torch.zeros((self.nbf,self.nbf))
        for i in range(20):
            #E_scf = self.hf_iter(H, A, G, Enuc)
            E_scf = cp(self.hf_iter, H, A, G, Enuc, row_idx=row_idx, preserve_rng_state=False)
            self.iter_count += 1
