...
class RHF(nn.Module):
    ...
    def forward(self, geom, row_idx):
        E_scf, eps, C, G = cp(self.everything, geom, row_idx=row_idx, preserve_rng_state=False)
        return E_scf, eps, C, G

hessian = RHF()
E_scf, eps, C, G = hessian(mygeom, 2)
E_scf.backward(create_graph=True, retain_graph=True)
...
#mp2_c = mp2_corr(eps, C, G, torch.tensor(1))
mp2_c = cp(mp2_corr, eps, C, G, torch.tensor(1))
print(mp2_c)
mp2_c.backward(create_graph=True)
print(mygeom.grad.clone().flatten())

#E_mp2 = mp2_c + E_scf

#E_mp2.backward(create_graph=True, retain_graph=True)
#E_mp2.backward(retain_graph=True) #create_graph=True, retain_graph=True)
#hessian_row = mygeom.grad.clone().flatten()
#print(hessian_row)
#mygeom.grad.zero_()
