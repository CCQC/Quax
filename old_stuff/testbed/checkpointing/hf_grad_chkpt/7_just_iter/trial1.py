...
#basis4 = torch.rand(25)
#basis4 = basis4.repeat(4)
...
class RHF(nn.Module):
    ...
    def forward(self, geom, row_idx):
        ...
        for i in range(10):
            E_scf, eps, C, D = cp(self.everything, geom, D, row_idx=row_idx, preserve_rng_state=False)
            #E_scf, eps, C = self.everything(geom)
            print(E_scf)
