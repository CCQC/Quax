        ...
        for _ in s.while_range(lambda: s.lp > -1):
          ...
          for _ in s.while_range(lambda: s.rp > -1):
            ...
            for _ in s.while_range(lambda: s.i > -1):
              ...
              #gx = (s.RP[0] - s.RQ[0])**(s.l - 2 * s.r + s.lp - 2 * (s.rp + s.i))
              #gx = (s.RP[0] - s.RQ[0])**(s.l - 2 * s.r + s.lp - 2 * (s.rp + s.i))
              #gx = np.dot(s.RP,s.RQ)**(s.l - 2 * s.r + s.lp - 2 * (s.rp + s.i))
              #gx = s.l - 2 * s.r + s.lp - 2 * (s.rp + s.i)
              #gx = 1
              s.m = s.mA + s.mB 
                            ...
                            for _ in s.while_range(lambda: s.np > -1):
                              ...
                              for _ in s.while_range(lambda: s.tp > -1):
                                ...
                                for _ in s.while_range(lambda: s.k > -1):
                                  ...
                                  #F = 1  #TODO
                                  #boys(nu, s.boysarg) #TODO fix this when JAX fixes the vmap issue with igamma
                                  s.Gxyz += F * gx * gy * gz # this is working
                                  #F = boys_evals[nu]
                                  #, s.boysarg) #TODO fix this when JAX fixes the vmap issue with igamma
                                  #s.Gxyz += F * gx * gy * gz # this is working
