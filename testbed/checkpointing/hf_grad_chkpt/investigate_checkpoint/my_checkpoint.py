...
class HessCheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *args):
        ...
        if len(inputs) == 1:
            gradient = detached_inputs[0].grad.clone()
            print(gradient)
            detached_inputs[0].grad.zero_()
            gradient.flatten()[ctx.row_idx].backward(create_graph=True)
        ...
        #print(gradients)
        ##torch.autograd.backward(outputs, args, create_graph=True)
        #torch.autograd.backward(tuple(gradients), args, create_graph=True)
        #if len(inputs) == 1:
        #    gradient = detached_inputs[0].grad.clone()
        #torch.autograd.backward(outputs, args, create_graph=True)
        #if len(inputs) == 1:
        #    gradient = detached_inputs[0].grad.clone()
        ##[inp.grad.zero_() for inp in detached_inputs if inp.grad]
        #if len(inputs) == 1:
        #    #gradient = detached_inputs[0].grad.clone()
        #    #print(gradient)
        #    detached_inputs[0].grad.zero_()
        #    #gradient.flatten()[ctx.row_idx].backward(retain_graph=True)
        #    gradient.flatten()[ctx.row_idx].backward(create_graph=True)
