...
#print('indices',indices.shape)
    ...
    #G = jax.lax.scan(partial(general, am='ssss', geom=geom), init_carry, np.arange(idx_i.shape[0]))
def general(carry, i, am, geom):
    G, tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,tmp7 = carry
    centers, exps, coeff, idx_i, idx_j, idx_k, idx_l = tmp1[i],tmp2[i],tmp3[i],tmp4[i],tmp5[i],tmp6[i],tmp7[i]
    ...
    G = jax.ops.index_update(G, (idx_i,idx_j,idx_k,idx_l), contracted)
    #new_carry = (G, centers, exps, coeff, idx_i, idx_j, idx_k, idx_l)
    new_carry = (G, tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,tmp7)
    return new_carry, contracted
...
def compute(geom, basis_dict, shell_quartets):
    ...
    # Nintegrals by 3
    A_all, B_all, C_all, D_all = geom[centers[:,0]], geom[centers[:,1]],geom[centers[:,2]],geom[centers[:,3]]
    ...
    def fill_G_ssss(G):
        ...
        def ssss_scan(carry, i):
            G, exps_all, coeffs_all, A_all, B_all, C_all, D_all, idx_i_all, idx_j_all, idx_k_all, idx_l_all = carry 
            exps = exps_all[i]
            coeff = coeffs_all[i]
            idx_i, idx_j, idx_k, idx_l = idx_i_all[i], idx_j_all[i], idx_k_all[i], idx_l_all[i]
            coeff = np.prod(cartesian_product(coeff[0,:],coeff[1,:],coeff[2,:],coeff[3,:]), axis=1)
            exps = cartesian_product(exps[0,:],exps[1,:],exps[2,:],exps[3,:])
            A, B, C, D = A_all[i], B_all[i], C_all[i], D_all[i]
            aa, bb, cc, dd = exps[:,0], exps[:,1], exps[:,2], exps[:,3]

            primitives = ssss(A,B,C,D,aa,bb,cc,dd,coeff)
            contracted = np.sum(primitives, axis=0).reshape(-1)
            G = jax.ops.index_update(G, (idx_i,idx_j,idx_k,idx_l), contracted)
            new_carry = (G, exps_all, coeffs_all, A_all, B_all, C_all, D_all, idx_i_all, idx_j_all, idx_k_all, idx_l_all)
            return new_carry, 0

        idx_i, idx_j, idx_k, idx_l = indices[slices[0],:sizes[0],0], indices[slices[0],:sizes[0],1], indices[slices[0],:sizes[0],2], indices[slices[0],:sizes[0],3]
        init_carry = (G, exps, coeffs, A_all, B_all, C_all, D_all, idx_i, idx_j, idx_k, idx_l)
        junk, _ = jax.lax.scan(ssss_scan, init_carry, np.arange(idx_i.shape[0]))
        G = junk[0]
        return G
    G = fill_G_ssss(G)
    ...
    #    print(am_classes[i])
    #    idx_i, idx_j, idx_k, idx_l = indices[slices[i],:sizes[i],0],indices[slices[i],:sizes[i],1], indices[slices[i],:sizes[i],2], indices[slices[i],:sizes[i],3]
    #    init_carry = (G, centers[slices[i]], exps[slices[i]], coeffs[slices[i]], idx_i, idx_j, idx_k, idx_l)
    #    junk, _ = jax.lax.scan(partial(general, am=am_classes[i], geom=geom), init_carry, np.arange(idx_i.shape[0]))
    #    G = junk[0]
