...
def homogenize_basisdict(basis_dict, max_prim):
    ...
    for i in range(len(basis_dict)):
        ...
        new_dict[i]['coef'] = onp.asarray(onp.pad(current_coef, (0, max_prim - current_coef.shape[0]), constant_values=0))
...
def general(data,am,geom):
    ...
    # Compute dummy padded values to save compile time, but waste computation
    ssss = jax.vmap(eri_ssss,(None,None,None,None,0,0,0,0,0))
    psss = jax.jit(jax.vmap(eri_psss,(None,None,None,None,0,0,0,0,0)))
    ppss = jax.vmap(eri_ppss,(None,None,None,None,0,0,0,0,0))
    psps = jax.jit(jax.vmap(eri_psps,(None,None,None,None,0,0,0,0,0)))
    ppps = jax.jit(jax.vmap(eri_ppps,(None,None,None,None,0,0,0,0,0)))
    pppp = jax.vmap(eri_pppp,(None,None,None,None,0,0,0,0,0))
