...
def debug(geom, coeffs, exps, atoms, am):
    ...
    #TODO DEBUG
                ...
                for l in range(nshells):
                    ...
                    ##G = jax.ops.index_add(G, (indices1, indices2, indices3, indices4), val.reshape(-1))
                    #index_combos = cartesian_product(indices1,indices2,indices3,indices4)
                    #G = jax.ops.index_update(G, (index_combos[:,0], index_combos[:,1], index_combos[:,2], index_combos[:,3]), val.reshape(-1))
