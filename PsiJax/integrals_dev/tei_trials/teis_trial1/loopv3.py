...
#                a = cartesian_product(indices[i],indices[j],indices[k],indices[l])
#                print(a[np.all(a > -1, axis=1)])
# These are all (shell dimension) by (contraction size) OR (angular momentum size) in the case of indices
# leading axis is the loop dim
print(coeffs.shape)
print(exps.shape)
print(indices.shape)
...
def compute(geom, coeffs, exps, atoms, am, indices):
    ...
    with loops.Scope() as s:
        def primitive(A, B, C, D, aa, bb, cc, dd, coeff, am):
            ...
            with loops.Scope() as S:
                ...
                for _ in S.cond_range(np.allclose(am,np.array([0,0,1,0]))): # WRONG TODO
                    primitive = np.where(coeff == 0,  0.0, eri_psss(*args)).reshape(1,1,3,1)
                for _ in S.cond_range(np.allclose(am,np.array([0,0,0,1]))): # WRONG TODO
                    primitive = np.where(coeff == 0,  0.0, eri_psss(*args)).reshape(1,1,1,3)

                #for _ in S.cond_range(np.allclose(am,np.array([1,1,0,0]))):
                #    primitive = np.where(coeff == 0,  0.0, eri_ppss(*args)).reshape(3,3,1,1)
                #for _ in S.cond_range(np.allclose(am,np.array([0,1,1,0]))): #WRONG TODO 
                #    primitive = np.where(coeff == 0,  0.0, eri_ppss(*args)).reshape(1,3,3,1)
                #for _ in S.cond_range(np.allclose(am,np.array([0,0,1,1]))): #WRONG TODO 
                #    primitive = np.where(coeff == 0,  0.0, eri_ppss(*args)).reshape(1,1,3,3)

                #for _ in S.cond_range(np.allclose(am,np.array([1,0,1,0]))):
                #    primitive = np.where(coeff == 0,  0.0, eri_psps(*args)).reshape(3,1,3,1)
                #for _ in S.cond_range(np.allclose(am,np.array([0,1,0,1]))): #WRONG TODO
                #    primitive = np.where(coeff == 0,  0.0, eri_psps(*args)).reshape(1,3,1,3)
                #for _ in S.cond_range(np.allclose(am,np.array([1,0,0,1]))): #WRONG TODO
                #    primitive = np.where(coeff == 0,  0.0, eri_psps(*args)).reshape(3,1,1,3)

                #for _ in S.cond_range(np.allclose(am,np.array([1,1,1,0]))):
                #    primitive = np.where(coeff == 0,  0.0, eri_ppps(*args)).reshape(3,3,3,1)
                #for _ in S.cond_range(np.allclose(am,np.array([1,1,0,1]))): #WRONG TODO
                #    primitive = np.where(coeff == 0,  0.0, eri_ppps(*args)).reshape(3,3,1,3)
                #for _ in S.cond_range(np.allclose(am,np.array([1,0,1,1]))): #WRONG TODO
                #    primitive = np.where(coeff == 0,  0.0, eri_ppps(*args)).reshape(3,1,3,3)
                #for _ in S.cond_range(np.allclose(am,np.array([0,1,1,1]))): #WRONG TODO
                #    primitive = np.where(coeff == 0,  0.0, eri_ppps(*args)).reshape(1,3,3,3)

                #for _ in S.cond_range(np.allclose(am,np.array([1,1,1,1]))):
                #    primitive = np.where(coeff == 0,  0.0, eri_pppp(*args)).reshape(3,3,3,3)
                return primitive
        ...
        def contraction(G, idx1, idx2, idx3, idx4, A, B, C, D, aa, bb, cc, dd, coeff, am):
            primitives = vectorized_primitive(A, B, C, D, aa, bb, cc, dd, coeff, am)
            contraction = np.sum(primitives, axis=0)
            indices1 = np.repeat(idx1, contraction.shape[0]) + np.arange(contraction.shape[0])
            indices2 = np.repeat(idx2, contraction.shape[1]) + np.arange(contraction.shape[1])
            indices3 = np.repeat(idx3, contraction.shape[2]) + np.arange(contraction.shape[2])
            indices4 = np.repeat(idx4, contraction.shape[3]) + np.arange(contraction.shape[3])
            index_combos = cartesian_product(indices1,indices2,indices3,indices4)
            G = jax.ops.index_update(G, (index_combos[:,0], index_combos[:,1], index_combos[:,2], index_combos[:,3]), contraction.reshape(-1))
            return G
            ...
            for j in s.range(nshells):
                ...
                for k in s.range(nshells):
                    ...
                    for l in s.range(nshells):
                        ...
                        s.G = contraction(s.G, idx1, idx2, idx3, idx4, A,B,C,D, 
                                          exp_combos[:,0], 
                                          exp_combos[:,1], 
                                          exp_combos[:,2],
                                          exp_combos[:,3],
