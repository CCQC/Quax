...
def general(data,am,geom):
    ...
    ##P = (aa * A + bb * B) * zetainv
    ##Q = (cc * C + dd * D) * etainv
    #P = (np.einsum('i,j->ij', aa, A) + np.einsum('i,j->ij', bb, B)) * np.transpose(np.tile(zetainv, (3,1)))
    #Q = (np.einsum('i,j->ij', cc, C) + np.einsum('i,j->ij', dd, D)) * np.transpose(np.tile(etainv, (3,1)))

    ## 81 x 3
    #PmQ = P - Q
    #boys_arg = (zeta * eta / (zeta + eta)) * np.einsum('ij,ji->i', PmQ, np.transpose(PmQ)) #jax.lax.dot(PmQ,PmQ)
...
#        print(i, G.flatten()[i], psi_G.flatten()[i])
#
