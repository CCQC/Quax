...
def general(data,am,geom):
    ...
    if am == 'ssss':
        args = (aa,bb,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, 0.0, eri_ssss(A,B,C,D,*args)), args)
    elif am == 'psss':
        args = (aa,bb,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros(3), eri_psss(A,B,C,D,*args)), args)
    elif am == 'spss':
        args = (bb,aa,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros(3), eri_psss(B,A,C,D,*args)), args)
    elif am == 'ssps':
        args = (cc,dd,aa,bb,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros(3), eri_psss(C,D,A,B,*args)), args)
    elif am == 'sssp':
        args = (dd,cc,bb,aa,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros(3), eri_psss(D,C,B,A,*args)), args)
    elif am == 'ppss':
        args = (aa,bb,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3)), eri_ppss(A,B,C,D,*args)), args)
    elif am == 'sspp':
        args = (cc,dd,aa,bb,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3)), eri_ppss(C,D,A,B,*args)), args)

    elif am == 'psps':
        args = (aa,bb,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3)), eri_psps(A,B,C,D,*args)), args)
    elif am == 'spps':
        args = (bb,aa,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3)), eri_psps(B,A,C,D,*args)), args)
    elif am == 'pssp':
        args = (aa,bb,dd,cc,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3)), eri_psps(A,B,D,C,*args)), args)
    elif am == 'spsp':
        args = (bb,aa,dd,cc,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3)), eri_psps(B,A,D,C,*args)), args)
    ...
    # You must transpose the result when you permute in some cases 
        ...
        args = (aa,bb,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3,3)), eri_ppps(A,B,C,D,*args)), args)
    elif am == 'ppsp': 
        args = (aa,bb,dd,cc,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3,3)), eri_ppps(A,B,D,C,*args)), args)
    elif am == 'pspp': 
        args = (cc,dd,aa,bb,coeff)
        primitives = np.transpose(jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3,3)), eri_ppps(C,D,A,B,*args)), args), (0,3,1,2))
    elif am == 'sppp': 
        args = (dd,cc,bb,aa,coeff)
        primitives = np.transpose(jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3,3)), eri_ppps(D,C,B,A,*args)), args), (0,3,2,1))
    elif am == 'pppp':
        args = (aa,bb,cc,dd,coeff)
        primitives = jax.lax.map(lambda args: jax.lax.select(args[-1] == 0.0, np.zeros((3,3,3,3)), eri_pppp(A,B,C,D,*args)), args)
...
def preprocess2(basis_dict):
    ...
    return am_classes, centers, exps, coeffs, indices, slices, sizes


#@partial(jax.jit, static_argnums=(1,6,7))
def compute(geom, am_classes, centers, exps, coeffs, indices, slices, sizes):
    G = np.zeros((nbf,nbf,nbf,nbf))
    for i in range(len(am_classes)):
        s = slices[i]
        size = sizes[i]
        print(am_classes[i])
        eris = jax.lax.map(partial(general, am=am_classes[i], geom=geom), (centers[s], exps[s], coeffs[s]))
        G = jax.ops.index_update(G, (indices[s,:size,0],indices[s,:size,1],indices[s,:size,2],indices[s,:size,3]), eris)
    return G
        
am_classes, centers, exps, coeffs, indices, slices, sizes = preprocess2(basis_dict)
G = compute(geom, am_classes, centers, exps, coeffs, indices, slices, sizes)
mints = psi4.core.MintsHelper(basis_set)
psi_G = np.asarray(onp.asarray(mints.ao_eri()))
print(np.allclose(G, psi_G))

#grad = jax.jacfwd(compute)(geom, am_classes, centers, exps, coeffs, indices, slices, sizes)
#print(grad.shape)
hess = jax.jacfwd(jax.jacfwd(compute))(geom, am_classes, centers, exps, coeffs, indices, slices, sizes)
print(hess.shape)
