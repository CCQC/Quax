...
def boys0(x):
    ...
    # TODO this causes memory blow up in JIT, I guess its tracing the full 'boys' array
...
# TODO jsut use 4term taylor instead of 6? not differentiating to get higher am integrals anyway

def boys1(x):
    return -jax.grad(boys0)(x)

def boys2(x):
    return -jax.grad(boys1)(x)

def boys3(x):
    return -jax.grad(boys2)(x)

def boys4(x):
    return -jax.grad(boys3)(x)
...
#    #Assume fused factorial factors/minus signs, preslice, convert to lax ops 
#    s = boys[:,i]
#    F = s[1] + 2 * xx * s[2] + 3*xx**2 * s[3] + 4 * xx**3 * s[4] + 5 * xx**4 * s[5]
#    return F 
#
#def boys2(x):
#    interval = 1e-5 # The interval of the precomputed Boys function grid
...
#    s = boys[:,i]
#    F = 2 * s[2] + 6*xx * s[3] + 12 * xx**2 * s[4] + 20 * xx**3 * s[5]
#    return -jax.grad(boys1)(x)
