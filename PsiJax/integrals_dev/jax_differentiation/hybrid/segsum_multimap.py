...
def preprocess(geom, basis_dict, nshells):
    ...
    for i in range(nshells):
        ...
        for j in range(nshells):
            ...
            # For segment sum over all primitives, each primitive needs a unique identifier
            size = ((bra_am + 1) * (bra_am + 2) // 2) *  ((ket_am + 1) * (ket_am + 2) // 2) 
            identifiers = onp.arange(size) + seg_id
    ...
    segment_ids = np.array(onp.asarray(segment_ids))

    primitive_locations = np.asarray(primitive_locations)
    return np.asarray(onp.asarray(basis_data)), centers_bra, centers_ket, primitive_locations, segment_ids

print("starting preprocessing")
a = time.time()
basis_data, centers1, centers2, primitive_locations, segment_ids = preprocess(geom, basis_dict, nshells)
b = time.time()
...
def build_overlap(geom, centers1, centers2, basis_data, primitive_locations):
    ...
    ##contracted = jax.ops.segment_sum(all_primitives,segment_ids)
    ## NOTE this is clearly wrong, but is index_update more efficient?
    #def update(carry, i):
    #    new_carry = jax.ops.index_update(carry, (primitive_locations[i,0], primitive_locations[i,1]), all_primitives[i])
    #    return new_carry, 0
    

    #new_S = jax.lax.scan(update, S, np.arange(900))


    #for i in range(900):
    #    new_S = jax.ops.index_update(S, (primitive_locations[i,0], primitive_locations[i,1]), all_primitives[i])
...
#print(grad.shape)
hess = jax.jacfwd(jax.jacfwd(build_overlap))(geom, centers1, centers2, basis_data, primitive_locations)
#print(hess)
