...
def build_overlap(geom, centers1, centers2, basis_data, am_data):
    ...
    #TODO TEMP
    centers_bra = np.repeat(geom[0].reshape(1,-1),3600, axis=0)
    centers_ket = np.repeat(geom[1].reshape(1,-1),3600, axis=0)
    #centers_ket = np.repeat(geom[1], 3600, axis=0) 
...
#    #def ssmap(centers_bra, centers_ket, basis_data):
    def ssmap(inp):
        ...
        #val = np.where(alpha_bra == 0, 0.0, V_overlap_pp(*args)[:,0,0])
        #val = np.where(alpha_bra == 0, 0.0, V_overlap_dp(*args)[:,0,0])
        val = np.where(alpha_bra == 0, 0.0, V_overlap_dd(*args)[:,0,0])
        return np.sum(val, axis=-1)
        #return np.where(alpha_bra == 0, 0.0, V_overlap_ss(*args))
        #vectorized_ss_primitives = jax.vmap(ssmap, (0,0,0))
    ss_contracted = jax.lax.map(ssmap, (centers_bra, centers_ket, basis_data))
